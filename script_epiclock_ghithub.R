...
plot_methylation_regression_lines_with_points <- function(ont_m_matrix_filtered, input_name = "data",
  ...
  # Slope filter: ascending or descending only. We separate PcGs best direct correlated with ageing from the ones inverse correlated.
  if (min_slope >= 0) {
    slope_filter <- ont_m_matrix_filtered$slope >= min_slope
  } else {
    slope_filter <- ont_m_matrix_filtered$slope <= min_slope
  ...
  actual_meth <- filtered_data[, c("X5", "X13", "X24", "X46", "X69", "X70", "X91")]
